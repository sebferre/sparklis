...
class boolean_input ~(input_selector : string) ~default () =
object
  ...
  method init =
	...
	input)
  method reset =
    if current_v <> init_v then begin
      jquery_input input_selector (fun input -> input##checked <- bool init_v);
      current_v <- init_v;
      has_changed <- true
    end
end

class integer_input ~(input_selector : string) ?min ?max ~default () =
object
  inherit config_input
  ...
  method reset =
    if current_v <> init_v then begin
      jquery_input input_selector (fun input -> input##value <- string (string_of_int init_v));
      current_v <- init_v;
      has_changed <- true
    end
end

class ['a,'h] lexicon_input
...
object (self)
  ...
  method private change_lexicon select input lang_input : unit =
    let _, p, l = self#get_select_property_lang select input lang_input in
  ...
  method reset =
    if current_property <> init_property || current_lang <> init_lang then begin
      jquery_select select_selector (fun select -> select##value <- string init_select);
      jquery_input input_selector (fun input -> input##value <- string (if init_select = other then init_property else ""));
      jquery_input lang_input_selector (fun input -> input##value <- string init_lang);
      current_property <- init_property;
      current_lang <- init_lang;
      has_changed <- true;
      self#define_lexicon
...
let logging = new boolean_input ~input_selector:"#input-logging" ~default:true ()

let max_results = new integer_input ~input_selector:"#input-max-results" ~min:1 ~default:200 ()
let max_classes = new integer_input ~input_selector:"#input-max-classes" ~min:0 ~default:200 ()
let max_properties = new integer_input ~input_selector:"#input-max-properties" ~min:0 ~default:200 ()
